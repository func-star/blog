...
可以看到Promise其实是个构造函数，原型上挂着catch和then方法，自身拥有race、all、reject、resolve等方法。
...
// 第一个异步请求开始
```
我们可以看到，Promise()接收一个包含两个参数的函数，两个参数分别为resolve和reject，对应着成功和失败。而且在实例化一个Promise的时候，回调函数会马上执行。
...
这里我们可以发现 `.then()` 中如果不返回一个 Promise 对象而是返回一个值，也能被下一个 `.then()` 方法接收到。
